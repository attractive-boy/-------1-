  ...
  <div class="claim-manage">
    <el-card shadow="never" class="box-card">
      ...
      <div class="filter-container">
        <el-input
          v-model="searchForm.itemTitle"
          placeholder="物品标题"
          clearable
          class="filter-item"
          style="width: 200px"
        />
        <el-select v-model="searchForm.status" placeholder="申请状态" clearable class="filter-item">
          <el-option label="全部" :value="null" />
          <el-option label="待审核" :value="0" />
          <el-option label="已通过" :value="1" />
          <el-option label="已拒绝" :value="2" />
        </el-select>
        <el-select v-model="searchForm.itemType" placeholder="物品类型" clearable class="filter-item">
          <el-option label="全部" :value="null" />
          <el-option label="招领物品" :value="0" />
          <el-option label="失物信息" :value="1" />
        </el-select>
        <el-button type="primary" @click="fetchClaims">查询</el-button>
      </div>
      
      <el-table v-loading="loading" :data="tableData" style="width: 100%">
        <el-table-column prop="itemTitle" label="物品标题" min-width="150" show-overflow-tooltip />
        ...
        <el-table-column prop="auditName" label="审核人" width="120" />
        <el-table-column prop="auditTime" label="审核时间" width="170" :formatter="formatAuditDate" />
    ...
    >
      <div v-if="currentClaim">
        ...
        <div class="detail-item">
          ...
          <span>{{ currentClaim.name || currentClaim.username }}</span>
        </div>
        <div class="detail-item">
        ...
        <div v-if="currentClaim.status !== 0" class="detail-item">
          ...
          <span>{{ formatAuditDate(currentClaim) }}</span>
        </div>
        <div v-if="currentClaim.status !== 0" class="detail-item">
...
const auditType = ref(0); // 1: 通过, 2: 拒绝
const auditForm = ref({
  id: null,
  status: null,
  auditRemark: ''
...
const fetchClaims = async () => {
  loading.value = true;
  try {
    await request.get('/claim/page', {
      currentPage: currentPage.value,
      size: pageSize.value,
      status: searchForm.value.status,
      itemType: searchForm.value.itemType,
      itemTitle: searchForm.value.itemTitle || undefined
    }, {
      onSuccess: (res) => {
...
// 格式化审核日期
const formatAuditDate = (row) => {
  return row.auditTime ? row.auditTime.replace('T', ' ').substring(0, 19) : '';
};
