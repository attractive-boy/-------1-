  ...
  <view class="page">
    ...
    <view class="search-area">
      ...
      <scroll-view scroll-x class="category-scroll">
        <view
          :class="['category-item', !searchForm.categoryId ? 'category-active' : '']"
        ...
        <view
          ...
          :key="cat.id"
          :class="['category-item', searchForm.categoryId === cat.id ? 'category-active' : '']"
    ...
    <view class="publish-bar" v-if="isLoggedIn">
      <button class="btn-publish" @click="goPublish">➕ 发布失物信息</button>
    </view>
    ...
    <view v-else-if="!lostItems.length" style="padding: 80rpx 0; text-align: center;">
      <text style="color:#999; font-size:28rpx;">暂无失物信息</text>
    </view>
    ...
    <view v-else class="item-list">
      ...
      >
        ...
        <view class="card-body">
          ...
          <text class="card-place">📍 {{ item.lostPlace || '未知地点' }}</text>
          <view class="card-footer">
            <text class="card-date">🕐 {{ formatTime(item.lostTime) }}</text>
    ...
    <!-- 加载更多 -->
    <view v-if="lostItems.length && lostItems.length < total" class="load-more" @click="loadMore">
      <text>加载更多 ({{ lostItems.length }}/{{ total }})</text>
    </view>
    <view v-if="lostItems.length && lostItems.length >= total && total > 0" style="text-align:center; padding: 20rpx; color:#999; font-size:24rpx;">
      <text>已加载全部</text>
    </view>
...
const fetchLostItems = async (reset = true) => {
  if (reset) {
    currentPage.value = 1
    lostItems.value = []
  }
  loading.value = true
  try {
    const res = await request.get('/lost-item/page', {
    ...
    if (res) {
      if (reset) {
        ...
        lostItems.value = [...lostItems.value, ...(res.records || [])]
      }
...
const viewDetail = (id) => uni.navigateTo({ url: `/pages/lost/detail?id=${id}` })
const goPublish = () => {
...
.search-input-wrap {
  display: flex; align-items: center; background: #f5f7fa; border-radius: 40rpx;
  padding: 0 24rpx; height: 80rpx; margin-bottom: 16rpx;
}
...
.category-item {
  display: inline-block; padding: 10rpx 28rpx; background: #f5f7fa;
  border-radius: 40rpx; margin-right: 16rpx; font-size: 26rpx; color: #666;
}
.category-active { background: #4A90D9; color: #fff; }

.publish-bar { padding: 20rpx; }
.btn-publish {
  background: #4A90D9; color: #fff; border-radius: 12rpx; font-size: 28rpx;
...
.item-card {
  display: flex; background: #fff; border-radius: 16rpx; margin-bottom: 20rpx;
...
.card-date { font-size: 22rpx; color: #aaa; }
.card-create { font-size: 22rpx; color: #bbb; }
